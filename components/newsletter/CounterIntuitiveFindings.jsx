...
export default function CounterIntuitiveFindings({ findings }) {
  const items = useMemo(
    () =>
      (findings || []).map((f) => ({
        headline: f.headline,
        narrative: f.finding,
        papers: f.paper_ids || f.papers || [],
      })),
    [findings]
  );
          ...
          <AnimatePresence initial={false} mode="wait">
            ...
            >
              ...
              <motion.div variants={itemVariants} className="min-w-0">
                <ResearchPaperCarousel papers={current.papers} section="Surprises" />
              </motion.div>
            </motion.article>
          </AnimatePresence>
        </div>
...
CounterIntuitiveFindings.propTypes = {
